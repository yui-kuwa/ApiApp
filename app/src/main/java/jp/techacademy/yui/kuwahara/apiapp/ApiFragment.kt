...
class ApiFragment: Fragment() {
    //ApiAdapterを後から初期化　　　requireContext()は非nullのContextを値として返す
    private val apiAdapter by lazy { ApiAdapter(requireContext()) }
    private val handler = Handler(Looper.getMainLooper())//別スレッドで動作させるためのもの
    ...
    /*        大事！！！        */
    private fun updateData(isAdd: Boolean = false) { //　Api通信を行い、データを取得するメソッド
        if (isLoading) {
            return
        } else {
            isLoading = true
        ...
        val client = OkHttpClient.Builder() //clientはHttp通信を行う本体
        ...
        val request = Request.Builder()
            .url(url)
            .build()
        client.newCall(request).enqueue(object : Callback { //ログに通信の詳細を出す
            override fun onFailure(call: Call, e: IOException) { // Error時の処理
                e.printStackTrace()
                handler.post { //別スレッドで行った処理をメインスレッドに教える
