...
class ApiAdapter (private val context: Context): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    private val items = mutableListOf<Shop>()//ApiResponseクラスのdata classであるShop classをリストに
    ...
    // ApiFragmentから表示リストの更新時に呼び出すメソッド
    // 表示リスト更新時に呼び出すメソッド
    ...
    //RecyclerViewで表示させる1件分のデータを作成　　　　RecycleViewの中身
    ...
    // RecyclerViewで表示させる1つ1つのセルのView　　　RecycleViewの見た目
    ...
    // RecyclerViewで表示させる（格納されている）件数を返すメソッド　　　RecycleViewの件数
    override fun getItemCount(): Int {
        // itemsプロパティに格納されている要素数を返す
        return items.size
    }

    // 第一引数にonCreateViewHolderで作られたViewHolder、第２引数に何番目の表示か
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ApiItemViewHolder) {
            // 生成されたViewHolderがApiItemViewHolderだったら。。。
            //             itemsからposition番のデータを取得して、ViewHolderの描画
            updateApiItemViewHolder(holder, position) //下で定義している関数
        }// {
    ...
    private fun updateApiItemViewHolder(holder: ApiItemViewHolder, position: Int) {
        ...
        holder.apply {
            ...
            // Picassoライブラリを使い、imageViewにdata.logoImageのurlの画像を読み込ませる（店の画像）
